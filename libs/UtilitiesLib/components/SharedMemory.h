...
// Pushes and pops the memory access mode. DON'T use this with WRITECOPY
...
// Returns true if the memory is currently locked by this process
bool sharedMemoryIsLocked(SharedMemoryHandle *handle);

// Commit changes about chunk and unlock manager, but keep individual chunk locked
...
//   acquired by another process, it can be easily read/written to without
...
SA_RET_OP_VALID void *sharedMemoryAlloc(SA_PARAM_NN_VALID SharedMemoryHandle *handle, size_t size);
...
extern bool g_bSharedMemoryPointerSet;

// Tests to see if a pointer is in a range that is from shared memory
int isSharedMemory_dbg(const void *ptr);

#define isSharedMemory(ptr) (g_bSharedMemoryPointerSet ? isSharedMemory_dbg(ptr) : false)
#define isSharedMemory2(ptr, ptr2) (g_bSharedMemoryPointerSet ? isSharedMemory_dbg(ptr) || isSharedMemory_dbg(ptr2) : false)

// Turns on WRITECOPY mode for all existing shared memory chunks, and causes future ones to acquire it
// Turn this on if you're modifying/reloading shared data. Otherwise, DON'T turn this on, as it can't be
// turned off again in this process
void sharedMemoryEnableEditorMode(void);

// Cancels all existing shared memory locks and enters edit mode
void sharedMemoryCancelAllLocks(void);

// Returns if we're in shared memory editmode (IE, WRITECOPY mode)
...
// For manually dealing with bytes alloced. Used for doing a simple shared malloc
...
void NoSharedMemory(int unused);

void sharedMemoryProcessCommandLine(void);


// Interface for a symbol shared named chunk of memory to use for inter-process
//  debugging (no locking mechanisms, so not robust for any production inter-process
//  communication).
void *sharedMemoryDebugGetScratch(const char *chunk_name, size_t size);

// We can not generate alerts during shared memory initialization because it happens too early in our startup process.
// This function will save any alert strings so that the alert can be triggered later.
void sharedMemoryQueueAlertString(const char *str);
...
// This function generates an alert from any alert strings that were queued
void sharedMemoryProcessQueuedAlerts(void);
#endif