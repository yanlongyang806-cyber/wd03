...
//#define STRING_TABLE_TRACK // Mostly useless now that we track to the original owner?
...
static MemChunk* pushNewMemChunk(StringTableImp *table, size_t uiSize MEM_DBG_PARMS )
{
	MemChunk* pNewChunk = scalloc(1, sizeof(MemChunk));
	pNewChunk->uiSize = uiSize;
#if !PLATFORM_CONSOLE
	if (table->virtualAlloc)
	{
		pNewChunk->pcData = VirtualAlloc(0, pNewChunk->uiSize, MEM_COMMIT | MEM_RESERVE, PAGE_READWRITE);
		//printfColor(COLOR_GREEN, "\nVirtualAlloc StringPool @%p\n", pNewChunk->pcData);
	} else
#endif
	{
		pNewChunk->pcData = scalloc( pNewChunk->uiSize, sizeof(char) );
	}
	return pNewChunk;
}

void initStringTableEx(StringTableImp* table, unsigned int chunkSize MEM_DBG_PARMS )
{
#ifdef STRING_TABLE_TRACK
	StringTableImp* pList = stringTableList;
	sprintf(table->cName, "ST: %s - %d", caller_fname, line);
	...
	table->pcFile = caller_fname;
	table->uiLine = line;

	assert(!table->readOnly);
	MAX1(chunkSize, DEFAULT_STARTING_MEM_CHUNK_SIZE);

	// We need to initialize the first, static memorychunk to the smallest size
	table->pFirstMemChunk = pushNewMemChunk(table, chunkSize MEM_DBG_PARMS_CALL);
}

StringTable strTableCreateEx(StringTableMode mode, unsigned int chunkSize MEM_DBG_PARMS)
{
	StringTableImp* pTable = scalloc(1, sizeof(StringTableImp));
	initStringTableEx(pTable, chunkSize MEM_DBG_PARMS_CALL);
...
StringTable strTableCopyEmptyToAllocatedSpace(StringTable table, void* pAllocatedSpace, size_t uiTotalSize )
{
	...
	pStringTableImp->pcFile = table->pcFile;
	pStringTableImp->uiLine = table->uiLine;
...
{
	int uiNumStrings = strTableGetStringCount(table);
	StringTable pStringTableImp = strTableCopyEmptyToAllocatedSpace(table, pAllocatedSpace, uiTotalSize);
	int uiStringIndex;
	...
	assert( pStringTableImp->pFirstMemChunk->uiCurrentIndex == pStringTableImp->pFirstMemChunk->uiSize ); // should be packed
	assert( pStringTableImp->pFirstMemChunk->pNextChunk == NULL ); // didn't add any memchunks!
...
StringTableImp* strTableCopyIntoSharedHeap(StringTableImp* table, const char* pcStringSharedHashKey, bool bAlreadyAreFirstCaller )
{
	...
	if ( ret == SHAR_FirstCaller )
	{
		...
		else
		{
			...
			StringTableImp* pStringTableImp;

			assert(pHandle);
			pStringTableImp = strTableCopyToAllocatedSpace(table, pHandle->data, uiTotalSize);
			assert(pStringTableImp);
...
void* strTableAddString(StringTableImp* table, const void* str)
{
	...
	if ( !newStringStartLocation )
	{
		...
		assert(pOldFirstChunk);
		uiSize = MIN(pOldFirstChunk->uiSize << 1, (table->virtualAlloc?(size_t)(4*1024*1024):MAX_MEMCHUNK_SIZE)); // the next chunk is bigger

		assert( !table->pSharedHeapHandle ); // can't add memchunks if we're in shared heap
		table->pFirstMemChunk = pushNewMemChunk(table, uiSize, table->pcFile, table->uiLine);
	...
	memcpy(newStringStartLocation, str, strSize);
	...
	if(table->indexable)
	{
		arrayPushBack_dbg(&table->indexTable, newStringStartLocation, table->pcFile, table->uiLine);
	}
...
int strTableSetMode(StringTableImp* table, StringTableMode mode)
{
	...
	if (mode & StrTableVirtualAlloc)
		table->virtualAlloc = 1;
...
{
	StringTableImp* pList = stringTableList;
	StashTable pStringTableHashTable = stashTableCreateWithStringKeys( 64, StashDeepCopyKeys_NeverRelease | StashCaseSensitive ); // whoo
