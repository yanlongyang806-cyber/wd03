...
void *listInsertAfter(void ** internallink, void * foreignlink)
{
	ListCast * oldlink;
	ListCast * newlink;
	...
	if(!foreignlink)
		return 0;
	oldlink = (ListCast*)*internallink;

	newlink = (ListCast*)foreignlink;

	newlink->prev = (ListCast *)*internallink;
	...
	//move internal link to new link
	*internallink = newlink;
