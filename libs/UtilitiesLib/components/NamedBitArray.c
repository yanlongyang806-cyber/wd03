...
static void nbdNameHashLazyInit(){
	if(!NamedBitArrayDefNameHash){
		NamedBitArrayDefNameHash = stashTableCreateWithStringKeys(4,  StashDeepCopyKeys_NeverRelease | StashCaseSensitive );
	}
}
...
static NamedBitArrayDefImp* createNamedBitArrayDefRaw(){
	...
	def->bitNameHash = stashTableCreateWithStringKeys(DEFAULT_NAMED_BIT_ARRAY_SIZE,  StashDeepCopyKeys_NeverRelease | StashCaseSensitive );
...
#pragma warning(suppress:4047) // return type differs in lvl of indirection warning
...
NamedBitArrayImp* createNamedBitArray(NamedBitArrayDefImp* def){
	...
	array->bits = baCreate(def->bitDefs.size);
	return array;
}
