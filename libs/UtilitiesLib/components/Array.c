...
AUTO_RUN_ANON(memBudgetAddMapping(__FILE__, BUDGET_EngineMisc);); // Should just be the memory pool
...
void arrayPushBackUnique_dbg(Array* array, void* element MEM_DBG_PARMS){
	int i = arrayFindElement(array, element);
	if(i != -1)
		return;

	arrayPushBack_dbg(array, element MEM_DBG_PARMS_CALL);
}

void arrayPushBackArray_dbg(Array* dst, Array* src MEM_DBG_PARMS){
	int i;

	for(i=0; i<src->size; i++)
		arrayPushBack_dbg(dst, src->storage[i] MEM_DBG_PARMS_CALL);
}
...
void arrayInsert(Array* array, unsigned int insertIdx, void* element)
{
	...
	arrayPushBack(array, NULL);

	if(insertIdx != (unsigned int)array->size - 1)
	{
		// Nudge everything after the new element forward.
		CopyStructs(array->storage + insertIdx + 1,
					array->storage + insertIdx,
					array->size - insertIdx - 1);
	}
...
int arrayFindAndRemoveFast(Array* array, void* element)
{
	int i = arrayFindElement(array, element);
	if(i != -1)
		arrayRemoveAndFill(array, i);
...
void* pqRemove(Array* queue, int index, QueueCompare comp, QueueIndexUpdate indexUpdate){
	void* removeItem;
	...
	removeItem = queue->storage[index];

	// move the item with the lowest ranking to the top
	queue->storage[index] = queue->storage[--queue->size];

	// restore heap order by perculating the lowest ranking item down
	percolateDown(queue, index, comp, indexUpdate);
	// if the target index is not 1, the moved element might be too low in its branch
	if(index != 1)
		pqPercolateUp(queue, index, comp, indexUpdate);
	return removeItem;
}

void* pqPop(Array* queue, QueueCompare comp, QueueIndexUpdate indexUpdate){
	return pqRemove(queue, 1, comp, indexUpdate);
}
...
void testHeap_IndexUpdate(void* item, int index)
{
	printf("IndexUpdate: item %d; index %d\n", (int)(intptr_t)item, index);
}
...
static void testHeap_Init(Array **array, int size)
{
	int i;
	*array = createArray();
...
static void testHeap_Print(Array *array)
{
	int i;
	for(i = 0; i < array->size; i++){
		printf("%p ", array->storage[i]);
	}
	printf("\n");
}

static void testHeap_Insert(Array *array, int value)
{
	pqPush(array, (void*)(intptr_t)value, (QueueCompare)intCompare, testHeap_IndexUpdate);
	printf("After inserting %d, the array looks like: ", value);
	testHeap_Print(array);
}

static void testHeap_Pop(Array *array)
{
	void* poppedItem;
	poppedItem = pqPop(array, (QueueCompare)intCompare, testHeap_IndexUpdate);
	printf("After extracting %d, the array looks like: ", (int)(intptr_t)poppedItem);
	testHeap_Print(array);
}

static void testHeap_Remove(Array *array, U32 index)
{
	void* removedItem;
	removedItem = pqRemove(array, index, (QueueCompare)intCompare, testHeap_IndexUpdate);
	printf("After extracting %d from position %u, the array looks like: ", (int)(intptr_t)removedItem, index);
	testHeap_Print(array);
}
...
void testHeap(){
	...
	testHeap_Init(&array, 11);	
	
	printf("Before heapifying, the array looks like: ");
	testHeap_Print(array);
	
	heapify(array, (QueueCompare)intCompare, testHeap_IndexUpdate);
	
	printf("After heapifying, the array looks like: ");
	testHeap_Print(array);
	
	testHeap_Insert(array, 11);

	testHeap_Pop(array);

	testHeap_Insert(array, 5);

	testHeap_Insert(array, 6);

	testHeap_Insert(array, 20);

	testHeap_Remove(array, 11);

	testHeap_Insert(array, 30);
 ...
 * 
 *
 * Begin Heap manipulation functions
 *************************************************************************/
