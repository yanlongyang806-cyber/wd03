...
char app_client_name[128] = "GameClient.exe";
...
int isExecutableInInstallDir(void)
{
	...
	if(result == -1)
	{
		...
		char		installationDir[CRYPTIC_MAX_PATH];
		char*		executableDir;
...
const char *getDevVersion(void)
{
	...
	ftime = fileLastChanged(name);
	_ctime32_s(SAFESTR(timestr), &ftime);
	sprintf(version,"dev: %s",timestr);
	version[strlen(version)-1] = 0;
	strcatf(version, " %d", fileGetGimmeDataBranchNumber());
	return version;
}

const char* getExecutablePatchVersionInternal(const char* projectName)
{
 ...
 *	This function tells whether the executable appear to be a patched version, and if so,
