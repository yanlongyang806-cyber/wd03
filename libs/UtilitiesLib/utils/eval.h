...
bool eval_Validate(EvalContext *pcontext, char *pchDbgName, char **ppchExpression, char **ppchOut);
	// Validates that the given expression is evaluatable by the context.

bool eval_Evaluate(EvalContext *pcontext, char **ppchExpression);
...
void eval_RegisterFunc(EvalContext *pcontext, char *pchName, EvalFunc pfn, int iNumIn, int iNumOut);
...
//    string stays valid which it's in the table.
