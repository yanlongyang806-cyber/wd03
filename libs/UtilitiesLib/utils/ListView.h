...
void listViewSetNoScrollToEndOnAdd(ListView *lv, bool scrollToEndOnAdd);
...
#define LISTVIEW_EMPTY_ITEM ((void*)-1) // To be used for a spacer
...
typedef void (*ListViewItemColorFunc)(ListView *lv, void *structptr, int row, COLORREF *clrText, COLORREF *clrTextBk);
typedef void (*ListViewSubItemColorFunc)(ListView *lv, void *structptr, int row, int column, COLORREF *clrText, COLORREF *clrTextBk);
...
// Puts check boxes in front of the listView items
...
void listViewDoingInitialBuild(ListView *lv, bool bDoingInitialBuild); // Calling this before and after doing the initial adds will give a very slight performance improvement


//you can set a custom compare function for a column, so that using the little clickies to sort by that column doesn't
//just alphabetize (or whatever)
