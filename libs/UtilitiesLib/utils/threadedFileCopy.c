...
typedef struct TFCRequestQueueEntry {
	...
	// For TFC_PROGRESS:
	size_t bytes; // bytes copied
	size_t total; // total file size
...
AUTO_RUN;
void tfcInitCriticalSections(void)
{
...
#define MAX_ALLOWED_MEM_USAGE (300*1024*1024)
static void tfcRecordMalloc(U64 size)
{
	...
	//printf("TFC: +%10"FORM_LL"d = %10"FORM_LL"d              \n", size, total_allocated);
...
void tfcRecordMemoryUsed(U64 size)
{
	...
	EnterCriticalSection(&critSectMallocTrack.critSect);
	total_allocated += size;
	//printf("TFCExternal: +%10"FORM_LL"d = %10"FORM_LL"d              \n", size, total_allocated);
	LeaveCriticalSection(&critSectMallocTrack.critSect);
}
...
static void recordBytesCopied(size_t bytes)
{
...
static int doFileCopy(TFCRequest* request)
{
	FWStatType status;
	FILE	*file;
	size_t		bytes_read=0, last_read, total;
	char	*mem=0;
	...
	if(0!=fwStat(request->src, &status)){
		return -1;
	}
...
{
	if (eaSize(&queue_results) < num+1)
		eaSetSize(&queue_results, num+1);
	if (eaSize(&queue_requests) < num+1)
		eaSetSize(&queue_requests, num+1);
	if (queue_results && queue_results[num]==NULL) {
		queue_results[num] = calloc(sizeof(*queue_results[num]), 1);
		queueInit(queue_results[num], THREAD_MAX_QUEUE_SIZE);
	}
	if (queue_requests && queue_requests[num]==NULL) {
