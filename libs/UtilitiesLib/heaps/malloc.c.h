// Used by nedmalloc, close to identical to what's in dlmalloc.c (slightly different version, included with different #defines?)
...
*/
static int mspace_mmap_large_chunks(mspace msp, int enable);
...
*/
static size_t mspace_usable_size(void* mem);
...
#if defined(_MSC_VER) && _MSC_VER>=1300 && !_XBOX
...
#elif defined(_MSC_VER) && _MSC_VER>=1300 && !_XBOX
#define compute_tree_index(S, I)\
{\
  size_t X = S >> TREEBIN_SHIFT;\
  if (X == 0)\
