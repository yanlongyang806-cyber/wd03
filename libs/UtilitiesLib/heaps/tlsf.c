 ...
 * - Added tlsf_get_used_size function (REMOVED in 2.4)
...
/******************************************************************/
    ...
    if (!mem_pool || !mem_pool_size || mem_pool_size < sizeof(tlsf_t) + BHDR_OVERHEAD * 8) {
        ERROR_MSG("tlsf_init_memory_pool (): memory_pool invalid\n");
        return (size_t) -1;
    }

    if (((unsigned long) mem_pool & PTR_MASK)) {
        ERROR_MSG("tlsf_init_memory_pool (): mem_pool must be aligned to a word\n");
        return (size_t) -1;
    }
    ...
    tlsf_free_ex(b->ptr.buffer, tlsf);
...
/******************************************************************/
size_t tlsf_add_new_area(void *area, size_t area_size, void *mem_pool)
{
/******************************************************************/
    ...
    ai->end = lb0;
    tlsf->area_head = ai;
    tlsf_free_ex(b0->ptr.buffer, mem_pool);
    return (b0->size & BLOCK_SIZE);
}


/******************************************************************/
size_t tlsf_get_used_size(void *mem_pool ATTRIBUTE_UNUSED)
{
/******************************************************************/
#if TLSF_STATISTIC
...
/******************************************************************/
size_t tlsf_get_max_size(void *mem_pool  ATTRIBUTE_UNUSED)
{
/******************************************************************/
#if TLSF_STATISTIC
...
/******************************************************************/
    ...
    if (!mp) {
        ...
        tlsf_init_memory_pool(area_size, area);
    }
#endif

    TLSF_ACQUIRE_LOCK(&((tlsf_t *)mp)->lock);

    ret = tlsf_malloc_ex(size, mp);
...
/******************************************************************/
    ...
    ret = tlsf_realloc_ex(ptr, size, mp);
...
/******************************************************************/
    ...
    ret = tlsf_calloc_ex(nelem, elem_size, mp);
...
/******************************************************************/
void *tlsf_malloc_ex(size_t size, void *mem_pool)
{
/******************************************************************/
    tlsf_t *tlsf = (tlsf_t *) mem_pool;
    bhdr_t *b, *b2, *next_b;
    ...
    if (!b) {
        ...
        if (area == ((void *) ~0))
            return NULL;        /* Not enough system memory */
        tlsf_add_new_area(area, area_size, mem_pool);
        /* Rounding up the requested size and calculating fl and sl */
        MAPPING_SEARCH(&size, &fl, &sl);
...
/******************************************************************/
void tlsf_free_ex(void *ptr, void *mem_pool)
{
/******************************************************************/
    tlsf_t *tlsf = (tlsf_t *) mem_pool;
    bhdr_t *b, *tmp_b;
    ...
    if (b->size & FREE_BLOCK) {
        ERROR_MSG ("tlsf_free_ex(): double free %p\n", ptr);
        return;
    }
#endif
...
/******************************************************************/
void *tlsf_realloc_ex(void *ptr, size_t new_size, void *mem_pool)
{
/******************************************************************/
    tlsf_t *tlsf = (tlsf_t *) mem_pool;
    void *ptr_aux;
    ...
    if (!ptr) {
        if (new_size)
            return (void *) tlsf_malloc_ex(new_size, mem_pool);
        if (!new_size)
            return NULL;
    } else if (!new_size) {
        tlsf_free_ex(ptr, mem_pool);
        return NULL;
    }
...
#ifdef CHECK_DOUBLE_FREE
    if (b->size & FREE_BLOCK) {
        ERROR_MSG ("tlsf_realloc_ex(): invalid pointer %p\n", ptr);
        return (void *) tlsf_malloc_ex(new_size, mem_pool);
    }
#endif
    ...
    if (!(ptr_aux = tlsf_malloc_ex(new_size, mem_pool))){
        return NULL;
    }      
...
/******************************************************************/
void *tlsf_calloc_ex(size_t nelem, size_t elem_size, void *mem_pool)
{
/******************************************************************/
    void *ptr;
    ...
    if (!(ptr = tlsf_malloc_ex(nelem * elem_size, mem_pool)))
        return NULL;
