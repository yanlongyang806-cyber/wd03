...
LONG WINAPI CustomUnhandledExceptionFilter(PEXCEPTION_POINTERS pExInfo)
{
	...
	handler->GenerateErrorReport(pExInfo, "0.0", "CustomUnhandledExceptionFilter",GetCurrentThreadId());
...
CCrashHandler::CCrashHandler(LPGETLOGFILE lpfn /*=NULL*/)
{
	...
	hRes = _Module.Init(NULL, GetModuleHandle("CrypticCrashRpt.DLL"));
	ATLASSERT(SUCCEEDED(hRes));
...
void CCrashHandler::GenerateErrorReport(PEXCEPTION_POINTERS pExInfo, const char *szVersion, const char *szMessage, DWORD dwThreadID)
{
	...
	m_files[rpt.getCrashLog(szVersion, szMessage)] = CString((LPCTSTR)IDS_CRASH_LOG);
	...
	if (dx8DiagnosticsStartup() == S_OK)
	{
		...
		dx8DiagnosticsCreate(&diagnostics, "Cryptic Studios Crash Report");
