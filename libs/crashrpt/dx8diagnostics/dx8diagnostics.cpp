...
HRESULT dx8DiagnosticsOutput::printWednesdaySpaced(char *string)
	...
	char buffer[256];
	char *trace = buffer;
	size_t trace_size = sizeof(buffer);
	bool addSpace = false;
	bool keepGoing = true;
	char charBuffer[2];
	charBuffer[1] = 0;
...
EXIT:
	...
	HRESULT findDLL(dx8DiagnosticsOutput *output, char *filename)
		...
		trace_size = filePath + sizeof(filePath) - trace;
		while ((*trace == '/') || (*trace == '\\')) {
			trace--;
			trace_size++;
		}
	...
	#define TRY_DIRECTORY(x) \
		strcpy_s(trace, trace_size, x "\\"); \
		strcat_s(trace, trace_size, filename); \
