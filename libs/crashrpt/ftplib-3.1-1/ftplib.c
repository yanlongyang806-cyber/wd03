...
static int socket_wait(netbuf *ctl)
{
	...
	do
	{
		...
		{
			rv = 0;
			strerror_s(ctl->ctrl->response, sizeof(ctl->ctrl->response), errno);
			break;
		}
		else if (rv > 0)
		{
...
static int readresp(char c, netbuf *nControl)
{
	...
	{
		strncpy_s(match,sizeof(match),nControl->response,3);
		match[3] = ' ';
		match[4] = '\0';
		do
		{
...
GLOBALDEF void FtpShutdown(void)
{
#if defined(_WIN32)
	if(wsockStarted)
		WSACleanup();
...
GLOBALDEF int FtpLogin(const char *user, const char *pass, netbuf *nControl)
{
	...
	if (((strlen(user) + 7) > sizeof(tempbuf)) ||
		...
		return 0;
	sprintf_s(tempbuf,sizeof(tempbuf),"USER %s",user);
	...
	sprintf_s(tempbuf,sizeof(tempbuf),"PASS %s",pass);
...
static int FtpOpenPort(netbuf *nControl, netbuf **nData, int mode, int dir)
{
	...
	{
		sprintf_s(nControl->response, sizeof(nControl->response), "Invalid direction %d\n", dir);
		return -1;
	}
	if ((mode != FTPLIB_ASCII) && (mode != FTPLIB_IMAGE))
	{
		sprintf_s(nControl->response, sizeof(nControl->response), "Invalid mode %c\n", mode);
		return -1;
	}
	l = sizeof(sin);
	if (nControl->cmode == FTPLIB_PASSIVE)
	{
		...
		if (cp == NULL)
			return -1;
		cp++;
		sscanf_s(cp,"%u,%u,%u,%u,%u,%u",&v[2],&v[3],&v[4],&v[5],&v[0],&v[1]);
	...
	else
	{
		...
		if (getsockname(sData, &sin.sa, &l) < 0)
			return 0;
		sprintf_s(buf, sizeof(buf), "PORT %d,%d,%d,%d,%d,%d",
...
static int FtpAcceptConnection(netbuf *nData, netbuf *nControl)
{
	...
	if (i == -1)
	{
		strerror_s(nControl->response, sizeof(nControl->response), errno);
	...
	{
		strcpy_s(nControl->response, sizeof(nControl->response), "timed out waiting for connection");
		net_close(nData->handle);
		nData->handle = 0;
		rv = 0;
	}
	else
	{
		if (FD_ISSET(nData->handle, &mask))
		{
			...
			{
				strerror_s(nControl->response, sizeof(nControl->response), i);
				nData->handle = 0;
				rv = 0;
			}
		}
						...
						netbuf **nData)
{
	...
	{
		sprintf_s(nControl->response, sizeof(nControl->response),
			"Missing path argument for file transfer\n");
		return 0;
	}
	sprintf_s(buf, sizeof(buf), "TYPE %c", mode);
	if (!FtpSendCmd(buf, '2', nControl))
		return 0;
	switch (typ)
	{
	case FTPLIB_DIR:
		strcpy_s(buf, sizeof(buf),"NLST");
		dir = FTPLIB_READ;
		break;
	case FTPLIB_DIR_VERBOSE:
		strcpy_s(buf, sizeof(buf),"LIST");
		dir = FTPLIB_READ;
		break;
	case FTPLIB_FILE_READ:
		strcpy_s(buf, sizeof(buf),"RETR");
		dir = FTPLIB_READ;
		break;
	case FTPLIB_FILE_WRITE:
		strcpy_s(buf, sizeof(buf),"STOR");
		dir = FTPLIB_WRITE;
		break;
	default:
		sprintf_s(nControl->response, sizeof(nControl->response), "Invalid open type %d\n", typ);
		return 0;
	}
	if (path != NULL)
	{
		...
		if ((strlen(path) + i) >= sizeof(buf))
			return 0;
		strcpy_s(&buf[i], sizeof(buf)-i,path);
	}
...
GLOBALDEF int FtpSite(const char *cmd, netbuf *nControl)
{
	...
	if ((strlen(cmd) + 7) > sizeof(buf))
		return 0;
	sprintf_s(buf,sizeof(buf),"SITE %s",cmd);
	if (!FtpSendCmd(buf,'2',nControl))
		return 0;
	return 1;
}
...
GLOBALDEF int FtpMkdir(const char *path, netbuf *nControl)
{
	...
	if ((strlen(path) + 6) > sizeof(buf))
		return 0;
	sprintf_s(buf,sizeof(buf),"MKD %s",path);
	if (!FtpSendCmd(buf,'2', nControl))
		return 0;
	return 1;
}
...
GLOBALDEF int FtpChdir(const char *path, netbuf *nControl)
{
	...
	if ((strlen(path) + 6) > sizeof(buf))
		return 0;
	sprintf_s(buf,sizeof(buf),"CWD %s",path);
	if (!FtpSendCmd(buf,'2',nControl))
		return 0;
	return 1;
}
...
GLOBALDEF int FtpRmdir(const char *path, netbuf *nControl)
{
	...
	if ((strlen(path) + 6) > sizeof(buf))
		return 0;
	sprintf_s(buf,sizeof(buf),"RMD %s",path);
	if (!FtpSendCmd(buf,'2',nControl))
		return 0;
	return 1;
}
...
GLOBALDEF int FtpSize(const char *path, int *size, char mode, netbuf *nControl)
{
	...
	if ((strlen(path) + 7) > sizeof(cmd))
		return 0;
	sprintf_s(cmd,sizeof(cmd), "TYPE %c", mode);
	if (!FtpSendCmd(cmd, '2', nControl))
		return 0;
	sprintf_s(cmd,sizeof(cmd),"SIZE %s",path);
	if (!FtpSendCmd(cmd,'2',nControl))
		rv = 0;
	else
	{
		if (sscanf_s(nControl->response, "%d %d", &resp, &sz) == 2)
			*size = sz;
		else
			rv = 0;
	}   
...
GLOBALDEF int FtpModDate(const char *path, char *dt, int max, netbuf *nControl)
{
	...
	if ((strlen(path) + 7) > sizeof(buf))
		return 0;
	sprintf_s(buf,sizeof(buf),"MDTM %s",path);
	if (!FtpSendCmd(buf,'2',nControl))
		rv = 0;
	else
		strcpy_s(dt, max, &nControl->response[4]);
	return rv;
}
...
GLOBALDEF int FtpRename(const char *src, const char *dst, netbuf *nControl)
{
	...
	if (((strlen(src) + 7) > sizeof(cmd)) ||
		...
		return 0;
	sprintf_s(cmd,sizeof(cmd),"RNFR %s",src);
	if (!FtpSendCmd(cmd,'3',nControl))
		return 0;
	sprintf_s(cmd,sizeof(cmd),"RNTO %s",dst);
	if (!FtpSendCmd(cmd,'2',nControl))
		return 0;
	return 1;
}
...
GLOBALDEF int FtpDelete(const char *fnm, netbuf *nControl)
{
	...
	if ((strlen(fnm) + 7) > sizeof(cmd))
		return 0;
	sprintf_s(cmd,sizeof(cmd),"DELE %s",fnm);
	if (!FtpSendCmd(cmd,'2', nControl))
		return 0;
	return 1;
}
