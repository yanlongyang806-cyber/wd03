...
int g_dumpingStack = 0;			// just a global to prevent recursion
...
int swStartup()
{
...

	{
		...
		strcpy_s( tt, TTBUFLEN, symSearchPath.c_str());
...
void ShowStackEx(StackDumpContext& sdContext)
{
	...
	for ( frameNum = 0; ; ++ frameNum )
	{
		...
		{ // we seem to have a valid PC
			...
			if ( ! pSGSFA( hProcess, s.AddrPC.Offset, &offsetFromSymbol, (PIMAGEHLP_SYMBOL)&SymPack ) )
			{
				if ( gle != 487 ) {
...
void enumAndLoadModuleSymbols( HANDLE hProcess, DWORD pid )
{
	...
	for ( it = modules.begin(); it != modules.end(); ++ it )
	{
		...
		int size = (*it).imageName.size() + 1;
		img = new char[size];
		strcpy_s( img, size, (*it).imageName.c_str() );
		size = (*it).moduleName.size() + 1;
		mod = new char[size];
		strcpy_s( mod, size, (*it).moduleName.c_str() );
