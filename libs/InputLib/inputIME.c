...
static bool s_HandleImeComposition( uiIMEState *s, HWND hWnd, DWORD dbcsChar, DWORD flgGcsType )
{
	...
	if( verify( s && hImc ))
	{
		...
		if( flgGcsType & GCS_COMPREADSTR )
		{
			...
			{
				U32 j;
				uiIMECandidateReading *candidate = &s->candidate;

				candidate->candidatesSize = MIN(bcLenStr/sizeof(buf[0]), ARRAY_SIZE(candidate->pszCandidates));
				ZeroArray(candidate->pszCandidates);

				// copy the characters to the entries
				// for the reading string, each character is an entry.
				for( j = 0; j < candidate->candidatesSize; ++j ) 
				{
					const char *tmp = WideToUTF8CodepointConvert(buf[j]);
					if( verify( tmp ))
					{
						strncpy(candidate->pszCandidates[j], tmp, ARRAY_SIZE(candidate->pszCandidates[j]) - 1 );
					}
				}
			}
...
// 				}
				...
				// updated with Aaron's approval
				strncpy_s(psz, ARRAY_SIZE( c->pszCandidates[j]) - 1, convertedTmp, _TRUNCATE);
			}
		}
...
static DWORD GetImeId( UINT uIndex )
{
    ...
    if( hklPrev == gInput->IME->input.keyboardLayout )
        return dwID[uIndex];

    hklPrev = gInput->IME->input.keyboardLayout; //s_hklCurrent;  // Save for the next invocation

    // Check if we are using an older Chinese IME
    if( !( ( gInput->IME->input.keyboardLayout == _CHT_HKL ) || ( gInput->IME->input.keyboardLayout == _CHT_HKL2 ) || ( gInput->IME->input.keyboardLayout == _CHS_HKL ) ) )
    {
        dwID[0] = dwID[1] = 0;
        return dwID[uIndex];
    }
    ...
    if ( !ImmGetIMEFileNameA( gInput->IME->input.keyboardLayout, szTmp, ( sizeof(szTmp) / sizeof(szTmp[0]) ) - 1 ) )
    {
        dwID[0] = dwID[1] = 0;
        return dwID[uIndex];
    }
        ...
        if( lpVerBuffer )
        {
            if( GetFileVersionInfoA( szTmp, dwVerHandle, dwVerSize, lpVerBuffer ) )
            {
                if( VerQueryValueA( lpVerBuffer, "\\", &lpVerData, &cbVerData ) )
                {
                    ...
                    if( 
                        ( LOWORD(gInput->IME->input.keyboardLayout) == LANG_CHT &&
                        ...
                        ||
                        ( LOWORD(gInput->IME->input.keyboardLayout) == LANG_CHS &&
                          ( dwVer == MAKEIMEVERSION(4, 1) ||
                            dwVer == MAKEIMEVERSION(4, 2) ||
                            dwVer == MAKEIMEVERSION(5, 3) ) )
                      )
                    {
                        dwID[0] = dwVer | LOWORD(gInput->IME->input.keyboardLayout);
...
//     s_CandList.dwSelection = (DWORD)-1; // do not select any char
    ...
    if( bUnicodeIme )
    {
        ...
        for( i = 0; i < dwReadingStrLen; ++i ) // dwlen > 0, if known IME
        {
			...
			strncpy( c->pszCandidates[i], WideToUTF8CodepointConvert(wstr[i]), ARRAY_SIZE(c->pszCandidates[i]) );
        }
    ...
    else
    {
        ...
        {
			char *nextStart = UTF8GetNextCodepoint(p+i);
...
static bool s_HandleImeNotify( uiIMEState * s, HWND hWnd, DWORD imnCmd, DWORD data)
{
	...
	if( verify( s && hImc ))
	{
		...
		{			
			...
			{
				GetPrivateReadingString( hWnd, gInput->IME );
			}
...
// 					{
// 						char *tmp = WideToUTF8CodepointConvert(buf[i]);
// 						if( verify( tmp ))
// 						{
				...
				for( i = 0; i < ARRAY_SIZE( imes ); ++i ) 
				{
					if( imes[i].id == wParam)
					{
						...
						if( wParam == IMN_SETCONVERSIONMODE )
						{
							...
							ImmGetConversionStatus( ImmGetContext(gInput->hwnd), &cstat, &sstat );
...
// 	break;
	case WM_INPUTLANGCHANGE:
	{
		res = !gInput->IME->pEdit
			|| s_HandleInputLangChange( gInput->IME, hWnd, wParam, (HKL)lParam );
	}
	break;
	case WM_IME_CHAR:
	...
	{
		res = !gInput->IME->pEdit
			|| s_HandleImeComposition( gInput->IME, hWnd, wParam, lParam );
	}
	break;
	case WM_IME_COMPOSITIONFULL:
	{
	...
	{
		res = !gInput->IME->pEdit
			|| s_HandleImeNotify(gInput->IME, hWnd, wParam, lParam );
	...
	case WM_IME_STARTCOMPOSITION:
	{
		...
		if( gInput->IME->pEdit )
		{
			gInput->IME->composition.visible = true;
			gInput->IME->composition.lenStr = 0;
		}
		res = true;
	}
	break;
	case WM_IME_ENDCOMPOSITION:
	{
		...
		if( gInput->IME->pEdit )
		{
//			UIEdit *edit = gInput->IME->pUIEdit;
			uiIMEComposition * c = &gInput->IME->composition;

			// --------------------
			// clean up candidate and composition information

			// hide the candidate and composition feedback
			gInput->IME->candidate.visible = kCandidateWindowVisibility_None;
			gInput->IME->composition.visible = false;

			// don't reset composition here, the cursor pos is needed for the result string.
			uiimecandidate_Reset(&gInput->IME->candidate);

			// clear the underline
/*			if( verify( gInput->IME->pUIEdit ))
			{
				uiEditSetUnderlineStateCurLine( gInput->IME->pUIEdit, false, 0, 0 );
			}*/
		}
		res = true;
	}
