...
static void cleanUpAStar(AStarSearchData* searchData)
{
	...
	while(searchData->infoListHead){
		AStarInfo* next = searchData->infoListHead->next;
		NODE_TO_ASTARINFO(searchData->infoListHead->ownerNode) = NULL;
		MP_FREE(AStarInfo, searchData->infoListHead);
		searchData->infoListHead = next;
	}
	assert(searchData->infoListHead==NULL);
	
	searchData->openSet = curOpenSet;
}

void AStarSearchCancel(AStarSearchData* searchData)
{
	cleanUpAStar(searchData);
}
...
void AStarSearch(AStarSearchData* searchData, NavSearchFunctions* functions){
	...
	if(!searchData->searching){
		...
		assert(!searchData->infoListHead);
		sourceInfo->next = searchData->infoListHead;
		searchData->infoListHead = sourceInfo;
	...
	while(1){
		...
		if(localFuncs.closeNode){
			localFuncs.closeNode(	searchData, sourceInfo->parentInfo ? sourceInfo->parentInfo->ownerNode : NULL,
		...
		prevNode = sourceInfo->parentInfo ? sourceInfo->parentInfo->ownerNode : NULL;
		connFromPrev = sourceInfo->connFromPrevNode;
			...
			PERFINFO_AUTO_START("Analyze Connections", 1);
				for(i = 0; i < count; i++){
					...
					if(!curInfo){
						...
						curCost = localFuncs.cost(searchData, prevNode, connFromPrev, sourceNode, connection);
						...
						curInfo->next = searchData->infoListHead;
						searchData->infoListHead = curInfo;
		...
		if(searchData->maxTicks)
		{
			...
			if(ticksCur - ticksStart > searchData->maxTicks)
				return;
		}
	}
