...
void dynClothColInitialize(DynClothCol *clothcol)
{
	...
	setVec3(clothcol->XVec, 1, 0, 0);
	setVec3(clothcol->YVec, 0, 1, 0);
	clothcol->YRadius = 1;
#endif
}

void dynClothColDelete(DynClothCol *clothcol)
{
	if (clothcol->Mesh)
		dynClothMeshDelete(clothcol->Mesh);
}
...
void dynClothColSetSphere(DynClothCol *clothcol, Vec3 p1, F32 r, F32 frac)
{
	Vec3 n;
	clothcol->Type = CLOTH_COL_SPHERE;
	setVec3(n, 0, 1.f, 0);
	dynClothColUpdate(clothcol, p1, n, 0, r, frac);
}
...
void dynClothColSetPlane(DynClothCol *clothcol, Vec3 c, Vec3 dir, F32 frac)
{
	...
	copyVec3(dir, n);
	//normalVec3(n);
	dynClothColUpdate(clothcol, c, n, 0, -1.0f, frac);
}

void dynClothColSetCylinder(DynClothCol *clothcol, Vec3 p1, Vec3 p2, F32 r, F32 exten1, F32 exten2, F32 frac)
{
	Vec3 n,c;
	F32 len,tlen;
	...
	len += (exten1 + exten2);
	dynClothColUpdate(clothcol, c, n, len, r, frac);
}

void dynClothColSetBalloon(DynClothCol *clothcol, Vec3 p1, Vec3 p2, F32 r, F32 exten1, F32 exten2, F32 frac)
{
	dynClothColSetCylinder(clothcol, p1, p2, r, exten1, exten2, frac);
...
void dynClothColSetBox(DynClothCol *clothcol, Vec3 p1, Vec3 norm, Vec3 xvec, Vec3 yvec, F32 height, F32 xrad, F32 yrad, F32 frac)
{
	...
	dynClothColUpdate(clothcol, p1, norm, height, xrad, frac);
	if (frac == 1.0f)
	{
...
F32 dynClothColCollideSphere(DynClothCol *clothcol, Vec3 p, F32 rad)
{
	...
	{
	  default:
	  case CLOTH_COL_SPHERE:	return dynClothColSphereSphereCol(clothcol, p, rad);
	  case CLOTH_COL_PLANE:		return dynClothColSpherePlaneCol(clothcol, p, rad);
	  case CLOTH_COL_CYLINDER:	return dynClothColSphereCylinderCol(clothcol, p, rad);
	  case CLOTH_COL_BALLOON:	return dynClothColSphereBalloonCol(clothcol, p, rad);
#if CLOTH_SUPPORT_BOX_COL
	  case CLOTH_COL_BOX:		return dynClothColSphereBoxCol(clothcol, p, rad);
#endif
	}
}
...
void dynClothColCreateMesh(DynClothCol *clothcol, int detail)
{
	...
	if (clothcol->Mesh)
		dynClothMeshDelete(clothcol->Mesh);
	clothcol->Mesh = dynClothMeshCreate();

	switch(clothcol->Type)
	{
	  default:
	  case CLOTH_COL_SPHERE:
		dynClothMeshPrimitiveCreateSphere(clothcol->Mesh, clothcol->Radius, detail);
		break;
	  case CLOTH_COL_PLANE:
		dynClothMeshPrimitiveCreatePlane(clothcol->Mesh, clothcol->Radius, detail);
		break;
	  case CLOTH_COL_CYLINDER:
		dynClothMeshPrimitiveCreateCylinder(clothcol->Mesh, clothcol->Radius, clothcol->HLength, detail);
		break;
	  case CLOTH_COL_BALLOON:
		dynClothMeshPrimitiveCreateBalloon(clothcol->Mesh, clothcol->Radius, clothcol->HLength, detail);
		break;
#if CLOTH_SUPPORT_BOX_COL
	  case CLOTH_COL_BOX:
		dynClothMeshPrimitiveCreateBox(clothcol->Mesh, clothcol->Radius, clothcol->YRadius, clothcol->HLength, detail);
		break;
#endif
	}
}
