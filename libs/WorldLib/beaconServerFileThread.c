...
void beaconServerWriteRequestedBeaconFile(BeaconProcessQueueNode* node){
	...
	beaconServerGetRequestFileName(SAFESTR(fileName), NULL, node->uniqueFileName, ".beaconrequest");
...
static BeaconFileLoadRequest* beaconServerGetNextLoadRequest(){
	BeaconFileLoadRequest* request = NULL;
...
static S32 beaconServerBeaconFileLoadThread(void* unused){
	...
	while(1){
		...
		if(request){
			...
			beaconServerGetRequestFileName(SAFESTR(fileName), NULL, request->input.fileName, ".beaconrequest");
...
void beaconServerInitLoadRequests(){
	...
	if(!init){
		...
		InitializeCriticalSection(&load_requests.cs);
		
		_beginthread(beaconServerBeaconFileLoadThread, 0, NULL);
...
static void beaconServerGetPendingFileName(	BeaconProcessQueueNode* node,
											char* fileName,
											S32 fileNameLen)
{
	beaconServerGetRequestFileName(fileName, fileNameLen, "pending", node->uniqueFileName, ".pending");
}
...
static void destroyBeaconProcessQueueNode(BeaconProcessQueueNode* node){
	...
	EnterQueueCS();
		...
		beaconServerGetPendingFileName(node, SAFESTR(fileName));
											...
											const char* dbServerIP)
{
	...
	estrCopy2(outStr, uniqueStorageName);
	...
	if(!beaconIsSharded() && dbServerIP){
		sprintf(buffer, "%s_", dbServerIP);
		estrInsert(outStr, 0, buffer, (S32)strlen(buffer));
	}
}
...
static S32 beaconServerAddProcessNodeToHashTable(BeaconProcessQueueNode* node){
	if(!beacon_server.processQueue.htFileNameToRequestNode){
		beacon_server.processQueue.htFileNameToRequestNode = stashTableCreateWithStringKeys(500, StashDeepCopyKeys_NeverRelease);
	}
											...
											U32 crc)
{
	...
	if(!node){
		...
		estrCopy2(&node->uniqueStorageName, uniqueStorageName);
...
static S32 beaconServerProcessQueueThread(void* unused){
	EXCEPTION_HANDLER_BEGIN
...
static FileScanAction queuePendingProcess(char* dir, struct _finddata32_t* data, void *pUserData){
	if(!(data->attrib & _A_SUBDIR) && strEndsWith(data->name, ".pending")){
		...
		estrCopy2(&node->uniqueFileName, fileName);
...
static void beaconServerLoadPendingRequests(){
	...
	fileScanAllDataDirs(pendingDir, queuePendingProcess, NULL);
...
void beaconServerInitProcessQueue(){
	...
	if(!init){
		...
		InitializeCriticalSection(&beacon_server.processQueue.cs);
		
		beaconServerLoadPendingRequests();
		
		beaconPrintf(COLOR_GREEN, "Starting beacon process queue thread: ");
		_beginthread(beaconServerProcessQueueThread, 0, NULL);
		beaconPrintf(COLOR_GREEN, "Done!\n");
	}
}
