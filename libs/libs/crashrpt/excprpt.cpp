...
CString CExceptionReport::getCrashFile()
{
   ...
   char temp[MAX_PATH];
   size_t retsize;
   getenv_s(&retsize, temp, sizeof(temp), "TEMP");

   // Create the dump file name
   sFile.Format(_T("%s\\%s.mdmp"), temp, CUtility::getAppName());
...
//
CString CExceptionReport::getCrashLog(const char *szVersion, const char *szMessage)
{
   CString sFile;
   ...
   sFile.Format(_T("%s\\%s.xml"), temp, CUtility::getAppName());
   V_VT(&v) = VT_BSTR;
