...
#if !_PS3
#pragma warning(disable:6285 6326 6246)
#endif
...
#if HAVE_MMAP
#define DEFAULT_MMAP_THRESHOLD (1024U * 1024U * 1024U * 2U) // allow huge reservations
#else
...
static void* allocateSharedPage(size_t size) {
	...
	assert( ret == SHAR_FirstCaller );

	ptr =  sharedMemorySetSize(shared_memory, size);
...
{\
  ...
  if (X == 0)\
    ...
    N += K = (((Y <<= K) - 0x4000) >> 16) & 2;\
    K = 14 - N + ((Y <<= K) >> 15);\
    I = (K << 1) + (unsigned int)((S >> (K + (TREEBIN_SHIFT-1)) & 1));\
  }\
}
#endif
...
static void internal_malloc_stats(mstate m) {
  if (!PREACTION(m)) {
    ...
    fprintf(stderr, "max system bytes = %10lIu\n", (maxfp));
    fprintf(stderr, "system bytes     = %10lIu\n", (fp));
    fprintf(stderr, "in use bytes     = %10lIu\n", (used));
...
/* NetLink a free chunk into a smallbin  */
...
#define replace_dv(M, P, S) {\
  ...
  if (DVS != 0) {\
    mchunkptr DV = M->dv;\
    assert(is_small(DVS));\
    insert_small_chunk(M, DV, ((bindex_t)(DVS)));\
  }\
  M->dvsize = S;\
  M->dv = P;\
}
...
#define insert_chunk(M, P, S)\
  if (is_small(S)) insert_small_chunk(M, P, ((bindex_t)(S)))\
  else { tchunkptr TP = (tchunkptr)(P); insert_large_chunk(M, TP, S); }

#define unlink_chunk(M, P, S)\
  if (is_small(S)) unlink_small_chunk(M, P, ((bindex_t)(S)))\
...
/* Malloc using mmap */
static void* mmap_alloc(__deref mstate m, size_t nb);
...
void* mspace_malloc(mspace msp, size_t bytes) {
  ...
  if (!PREACTION(ms)) {
    ...
    if (bytes <= MAX_SMALL_REQUEST) {
      ...
      binmap_t smallbits;
      nb = (bytes < MIN_REQUEST)? MIN_CHUNK_SIZE : pad_request(bytes);
      idx = (bindex_t)(small_index(nb));	// MAK - ok because we're smaller than MAX_SMALL_REQUEST
