...
MP_DEFINE(QueueImp);
Queue createQueue_dbg(MEM_DBG_PARMS_VOID){
	Queue ret;
	MP_CREATE(QueueImp,4);
	ret = (Queue)MP_ALLOC(QueueImp);
	MEM_DBG_STRUCT_PARMS_INIT(ret);
	return ret;
}
...
void initQueue(QueueImp* queue, int initialQueueSize){
	...
	queue->storage = stcalloc(initialQueueSize, sizeof(void*), queue);
...
int qResize(QueueImp* queue, int newSize){
	...
	newStorage = stcalloc(newSize, sizeof(void*), queue);
	if(!newStorage)
		return 0;
...
static void qDump(QueueImp* queue){
	...
	for(i = 0; i < queue->maxSize; i++){
		if(queue->storage[i]){
			printf("Index %i,	Value %p", i, queue->storage[i]);
		}else
