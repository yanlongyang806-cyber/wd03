...
__forceinline static unsigned int baGetStorageByteSize(unsigned int maxBitNum)
{
	unsigned int storageByteSize = (maxBitNum + 7) >> 3;
	if (storageByteSize > 0)
		storageByteSize -= 1;
	return storageByteSize;
}

__forceinline static void baSetSizeBytes(BitArrayImp** ba, unsigned int storageByteSize MEM_DBG_PARMS)
{
	if (*ba)
	{
		*ba = srealloc(*ba, sizeof(**ba) + storageByteSize);
		if ((*ba)->maxSetByte > storageByteSize)
		{
			(*ba)->maxSetByte = storageByteSize;
		}
		else if (storageByteSize > (*ba)->maxSize)
		{
			memset(&(*ba)->storage[(*ba)->maxSize+1], 0, storageByteSize - (*ba)->maxSize);
		}
	}
	else
	{
		*ba = scalloc(1, sizeof(**ba) + storageByteSize);
		(*ba)->maxSetByte = storageByteSize;
	}

	(*ba)->maxSize = storageByteSize;
}
...
{
	BitArrayImp* ba = NULL;
	baSetSizeBytes(&ba, baGetStorageByteSize(maxBitNum) MEM_DBG_PARMS_CALL);
	return ba;
}

void baDestroy(BitArrayImp* ba)
{
	SAFE_FREE(ba);
}

void baSetSize_dbg(BitArrayImp** ba, unsigned int maxBitNum MEM_DBG_PARMS)
{
	baSetSizeBytes(ba, baGetStorageByteSize(maxBitNum) MEM_DBG_PARMS_CALL);
}

void baSizeToFit_dbg(BitArrayImp** ba, unsigned int maxBitNum MEM_DBG_PARMS)
{
	unsigned int storageByteSize = baGetStorageByteSize(maxBitNum);

	if (!(*ba) || (*ba)->maxSize < storageByteSize)
		baSetSizeBytes(ba, pow2(storageByteSize) MEM_DBG_PARMS_CALL);
}

unsigned int baGetCapacity(BitArray ba)
{
	return (ba->maxSize+1) << 3;
}

int baSetBitValue(BitArrayImp* ba, unsigned int bitnum, int value)
{
	if(value)
		return baSetBit(ba, bitnum);
	else
		return baClearBit(ba, bitnum);
}
...
int baSetBitRange(BitArray ba, unsigned int bitnum_start, unsigned int num_bits){
	...
	else
	{
		...
		*byte |= byteMask;
		...
		for (i = 0; i < endBits; i++)
			byteMask |= 1 << i;
		byte = ba->storage + baIndexEnd;
		bitAltered = bitAltered || !(*byte & byteMask);
		*byte |= byteMask;
	}

	if (baIndexEnd > ba->maxSetByte)
		ba->maxSetByte = baIndexEnd;
 ...
 */
int baSetBit(BitArrayImp* ba, unsigned int bitnum){
 ...
 */
int baClearBit(BitArrayImp* ba, unsigned int bitnum){
	unsigned int baIndex;		// Given the bitnum, which byte should be examined?
	int bitAltered = 0;
	unsigned char* byte;
	int byteMask;
	...
	*byte &= ~byteMask;
	...
	if (baIndex == ba->maxSetByte)
	{
		while (*byte == 0 && ba->maxSetByte)
		{
			ba->maxSetByte--;
			byte = ba->storage + ba->maxSetByte;
		}
	}
...
int baClearBits(BitArrayImp* target, BitArrayImp* mask){
	...
	// CLEAR the allowed number of bytes.
	{
		...
		{
			if(!bitsAltered && (target->storage[i] & mask->storage[i]))
				bitsAltered = 1;

			target->storage[i] = target->storage[i] & ~mask->storage[i];
		}
	}
 ...
 */
void baSetAllBits(BitArrayImp* ba)
{
	assert(ba);

	memset(ba->storage, 0xffffffff, ba->maxSize + 1);

	ba->maxSetByte = ba->maxSize;

	//for(i = 0; i <= ba->maxSize; i++){
	//	((unsigned char*)&ba->storage)[i] = 0xff;
	//}
}

/* Function baClearAllBits()
 */
void baClearAllBits(BitArrayImp* ba)
{
	assert(ba);

	memset(ba->storage, 0, ba->maxSetByte + 1);

	ba->maxSetByte = 0;

	//for(i = 0; i <= ba->maxSize; i++){
	//	((unsigned char*)&ba->storage)[i] = 0;
	//}
}

/* Function baClearAllBits()
 */
void baNotAllBits(BitArrayImp* ba){
	unsigned int i;

	assert(ba);

	for(i = 0; i <= ba->maxSize; i++)
		ba->storage[i] = ~ba->storage[i];
 ...
 *		1 - Bit is set.
 */
int baIsSet(BitArrayImp* ba, unsigned int bitnum){
	unsigned int baIndex;		// Given the bitnum, which byte should be examined?
	assert(ba);
 ...
 *		1 - Bit is set.
 */
int baCountSetBits(BitArrayImp* ba){
	...
	for(byteIndex = 0; byteIndex <= ba->maxSetByte; byteIndex++){
		unsigned char byte = ba->storage[byteIndex];
...
int baGetMaxBits(BitArrayImp* ba){
	assert(ba);
...
int baOrArray(BitArrayImp* array1, BitArrayImp* array2, BitArrayImp* output){
	...
	// OR the allowed number of bytes.
	{
		unsigned int i;
		for(i = 0; i <= indexLimit; i++)
			output->storage[i] = array1->storage[i] | array2->storage[i];
	}
...
int baBitsAreEqual(BitArrayImp* array1, BitArrayImp* array2){
	...
	for(i = 0; i <= indexLimit; i++){
		if(array1->storage[i] != array2->storage[i])
			return 0;
	}
 ...
 *	This function turns all bits in a bit ba on, then off.  The target bit is 
...
void testBitArrayAllBits(unsigned int bitnum){
	...
	maxBits = baGetMaxBits(ba);
	printf("The BitArray can hold a maximum of %i bits\n", maxBits);

	// Test all the bits in the ba.
	{
		int i;
		int testFailed = 0;
...
void testBitArrayCountBits(int bitnum){
...

	{
		...
		// Set all bits in the ba.
		for(i = 0; i < baGetMaxBits(ba); i++){
			baSetBit(ba, i);
		}
	}
