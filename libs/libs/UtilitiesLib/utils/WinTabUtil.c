...
{ 
	...
	HGLOBAL hglb;
	assert(hrsrc); // invalid to call LoadResource with NULL
	hglb = LoadResource(hInst, hrsrc); 
	assert(hglb); // invalid to call LockResource with NULL
...
{ 
#pragma warning (suppress: 4312)	// GetWindowLongPtr produces a warning under /Wp64, but in fact correctly compiles for _x64
...
#pragma warning (suppress: 6386)	// Analyze is confused.
