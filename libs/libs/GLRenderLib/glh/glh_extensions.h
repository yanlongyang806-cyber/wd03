...
{
	int len = (int)(strlen(extName) + 2);
    char *padExtName = (char*)malloc(len);
	strcpy_s(padExtName, len, extName);
    strcat_s(padExtName, len, " ");
...
int glh_init_extensions(const char *origReqExts)
{
    ...
    if (NULL == g_gl_sysExts) {
...
#if defined(WIN32)
        {
            PFNWGLGETEXTENSIONSSTRINGARBPROC lwglGetExtensionsStringARB = 0;
            lwglGetExtensionsStringARB = (PFNWGLGETEXTENSIONSSTRINGARBPROC)wglGetProcAddress("wglGetExtensionsStringARB");
            if(lwglGetExtensionsStringARB)
            {
                winsys_extensions = lwglGetExtensionsStringARB(wglGetCurrentDC());
        ...
        g_gl_sysExts = (char*)malloc(sysExtsLen + winsysExtsLen + 3);
        CHECK_MEMORY(g_gl_sysExts);
        strcpy_s(g_gl_sysExts, sysExtsLen + winsysExtsLen + 3, extensions);
        g_gl_sysExts[sysExtsLen] = ' ';
        g_gl_sysExts[sysExtsLen + 1] = 0;
        strcat_s(g_gl_sysExts, sysExtsLen + winsysExtsLen + 3, winsys_extensions);
        g_gl_sysExts[sysExtsLen + 1 + winsysExtsLen] = ' ';
        g_gl_sysExts[sysExtsLen + 1 + winsysExtsLen + 1] = 0;
    }
...
#if (defined(WIN32) || defined(UNIX))
        if (!ExtensionExists(reqExt, g_gl_sysExts) || !glh_init_extension(reqExt)) {
#elif defined(MACOS)
        if (!ExtensionExists(reqExt, g_gl_sysExts)) {    // don't try to get function pointers if on MacOS
#endif
            // add reqExt to end of unsupportedExts
            strcat_s(unsupportedExts, reqExtsLen + 2, reqExt);
            strcat_s(unsupportedExts, reqExtsLen + 2, " ");
            success = GL_FALSE;
        }
        *extEnd = saveChar;
    }
...
int glh_extension_supported(const char *extension)
{
    ...
    for (;;) 
    {
        ...
        if (where == start || (where - 1 && *(where - 1) == ' '))
        {
