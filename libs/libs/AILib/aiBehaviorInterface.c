...
bool processingBehaviors = false;
...
AIBehaviorInfo* gameBehaviorTable = NULL;
size_t gameBehaviorTableEntries = 0;
...
void aiBehaviorProcessString(Entity* e, AIVarsBase* aibase, AIBehavior*** behaviors, const char* origStr, AIBParsedString* parsed);
...
bool aiBehaviorAddToHashTable(AIBTableEntry* entry)
{
...
void aiBehaviorTableAddBehavior(AIBehaviorInfo* info)
{
	...
	if(!aiBehaviorAddToHashTable(entry))
		SAFE_FREE(entry);
}
...
void aiBehaviorRebuildLookupTable()
{
	...
	eaClearEx(&BehaviorSystemTableList, NULL);

	aiBehaviorRegisterSystemBehaviors();

	if(gameBehaviorTable)
	{
		size_t i;
		for(i = 0; i < gameBehaviorTableEntries; i++)
			aiBehaviorTableAddBehavior(gameBehaviorTable + i);
	}
...
ParseTable ParseBehaviorString[] =
{
...
AIBParsedString* aiBehaviorParseString(const char* string)
{
	...
	estrCopy2(&estr, "behavior ");
	estrAppend2(&estr, string);
	...
	else
	{
		...
		char* escapebuf = ScratchAlloc(TOKEN_BUFFER_LENGTH);
		...
		while(cleanStart = strstri(cleanStart, "clean"))
		{
				...
				strchr(" (,<\"", *(cleanStart+5)))
			{
				...
				*cleanStart = '\0';
				...
				tok = TokenizerCreateString(buf, 0);
				break;
			}
			else
				cleanStart += 5;
		...
		if(buf && buf[0])
			...
			tok = TokenizerCreateString(estr, 0);


		parsed->string = NULL;

		ParserReadTokenizer(tok, ParseBehaviorString, parsed, false, NULL);

		TokenizerDestroy(tok);

		stashAddPointer(BehaviorParses, allocedStr, parsed, false);
		ScratchFree(escapebuf);
		return parsed;
	}
}
...
void aiBehaviorAddString(Entity* e, AIVarsBase* aibase, const char* string)
{
	...
	else
	{
		...
		// 		aiBehaviorDebugVerifyParse(string, parsed);
		eaPush(&aibase->behaviorPrevMods, mod);
		aiBehaviorProcessString(e, aibase, &aibase->behaviors, string, parsed);
		//aiBehaviorDestroyParsedString(&parsed);
	}
}
...
int aiBehaviorProcessAllStrings(Entity* e, AIVarsBase* aibase, AIBehavior*** behaviors)
{
	while(eaSize(&aibase->behaviorMods))
	{
		...
		else
		{
			...
			aiBehaviorProcessString(e, aibase, behaviors, mod->string, parsed);
			//aiBehaviorDestroyParsedString(&parsed);
		}
...
void aiBehaviorProcessString(Entity* e, AIVarsBase* aibase, AIBehavior*** behaviors, const char* origStr, AIBParsedString* parsed)
{
	...
	for(i = highestIdx-1; i >= 0 && continueProcessing; i--)
	{
		char* processedStr = parsed->string[i]->function;
		int addSelfAsFlag = true, processEntry = true;
		AIBTableEntry* entry;
...
int aiBehaviorFlagAddInternal(Entity* e, AIVarsBase* aibase, AIBehavior*** behaviors, AIBehavior* behavior, AIBehaviorFlagInfo* info, TokenizerFunctionCall* param)
{
	...
	// 			return false;
		...
		xcase FLAG_DATA_ESTR:
			WRONG_PARAMETERS(1, "(supposed to be 1 string)");
			estrCopy2(&flag->strptr, param->params[0]->function);
			estrConcatf(&estr, "(%s)", flag->strptr);
		xcase FLAG_DATA_FLOAT:
			if(eaSize(&param->params) != 1 && stricmp(param->params[0]->function, "rand"))
			{
		...
		xcase FLAG_DATA_MULT_STR:
			...
			{
				eaPush(&flag->strarray, allocAddString(param->params[i]->function));
			}
			estrConcatf(&estr, ")");
...
void aiBehaviorSetGameBehaviorTable(AIBehaviorInfo table[], size_t numentries)
{
	gameBehaviorTable = table;
	gameBehaviorTableEntries = numentries;
}
