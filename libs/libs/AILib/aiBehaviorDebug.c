...
char* aiBehaviorGetDebugString(Entity* e, AIVarsBase* aibase, AIBehavior* behavior, char* indent)
{
	...
	if(behavior->running)
		estrConcatf(&str[recursions], "^2");
	else if(!behavior->activated)
		estrConcatf(&str[recursions], "^8");
	else if(behavior->finished)
		estrConcatf(&str[recursions], "^1");
	else
		estrConcatf(&str[recursions], "^5");
...
char* aiBehaviorDebugPrintParse(AIBParsedString* parsed)
{
	...
	//	estrPrintf(&estr, "%s", parsed->string);
	estrPrintf(&estr, "");
	...
	for(i = 0; i < baseStrings; i++)
	{
		...
		for(j = 0; j < iChildren; j++)
		{
			...
			for(k = 0; k < jChildren; k++)
			{
				...
				{
					estrAppend2(&estr,
						aiBehaviorDebugPrintParseHelper(
						parsed->string[i]->params[j]->params[k]->params[l]->function));
					if(l < kChildren-1)
						estrAppend2(&estr, " ");
				}
				if(kChildren)
...
char* aiBehaviorPrintMod(AIBehaviorMod* mod)
{
	...
	timeMakeOffsetStringFromSeconds(timerstring, mod->time/3000);
...
void aiBehaviorFlagDebugString(Entity* e, AIBehavior* behavior, char** estr)
{
	...
	estrConcatf(estr, "^0Current Flags: ");
	for(i = 0; i < numflags; i++)
	{
		...
		case FLAG_DATA_ESTR:
			...
			estrConcatf(estr, "%s", AI_LOG_ENT_NAME(erGetEnt(flag->entref)));
			...
			estrConcatf(estr, "%s", StaticDefineIntRevLookup(flag->info->parseTable, flag->intarray[0]));
...
char* aiBehaviorDebugPrintPendingMods(Entity* e, AIVarsBase* aibase)
{
	...
	{
		estrConcatf(&buf, "%s", aiBehaviorPrintMod(aibase->behaviorMods[i]));
	}
...
char* aiBehaviorDebugPrintPrevMods(Entity* e, AIVarsBase* aibase)
{
	...
	{
		estrConcatf(&buf, "%s", aiBehaviorPrintMod(aibase->behaviorPrevMods[i]));
	}
