...
typedef struct BeaconGenerateEdgeNode {
	...
	BeaconGenerateEdgeNode*				prevEdgeNode;
...
typedef struct BeaconProcessConnection {
	...
	U32									optionalWalkCheck		: 1;
	U32									bidirWalkCheck			: 1;
} BeaconProcessConnection;

#define ANGLE_INCREMENT_MIN 2
#define ANGLE_INCREMENT_COUNT (360 / ANGLE_INCREMENT_MIN)
...
void beaconExtractSurfaces(WorldColl *wc, int grid_x, int grid_z, int dx, int dz, int justEdges);

void beaconAddToCheckList(BeaconGenerateFlatArea* flatArea, BeaconGenerateColumnArea* columnArea);

void beaconMakeBeaconsInChunk(int iPartitionIdx, BeaconGenerateChunk* chunk, int quiet);
