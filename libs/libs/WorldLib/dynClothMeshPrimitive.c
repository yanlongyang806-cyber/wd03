...
static void dynClothMeshPrimitiveCreateCylinderSub(DynClothMesh *mesh, F32 rad, F32 hlen, int detail, int balloon)
{
	int i,j,k,ndiv;
...

	{
		...
		dynClothMeshAllocate(mesh, NumRenderPoints);
		...
		// Create Indices
		{
			...
			dynClothMeshCreateStrips(mesh, nstrips , CLOTHMESH_TRISTRIP);

			// cylinder
			if (hlen > 0)
			{
				idx = 0;
				dynClothStripCreateIndices(&mesh->Strips[sidx],ndiv*2+2,-1);
				for (j=0; j<ndiv; j++)
				{
					assert(idx < mesh->Strips[sidx].NumIndices);
			...
			for (k=0; k<2; k++)
			{
				...
				int base_index;
				idx = 0;
				dynClothStripCreateIndices(&mesh->Strips[sidx], ndiv*2+1, CLOTHMESH_TRISTRIP);
				base_index = first + ndiv*(2-k) + k;
				for (j=0; j<ndiv; j++)
				{
					assert(idx < mesh->Strips[sidx].NumIndices);
					mesh->Strips[sidx].IndicesCCW[idx++] = first + (k ? j : ndiv-j-1);
					assert(idx < mesh->Strips[sidx].NumIndices);
					mesh->Strips[sidx].IndicesCCW[idx++] = base_index;
				}
				assert(idx < mesh->Strips[sidx].NumIndices);
				mesh->Strips[sidx].IndicesCCW[idx++] = first + (k ? 0 : ndiv-0-1);
				sidx++;
			}
			...
			dynClothMeshSetColorAlpha(mesh, clr, 0.75f);
		}
	}
}

void dynClothMeshPrimitiveCreateCylinder(DynClothMesh *mesh, F32 rad, F32 hlen, int detail)
{
	dynClothMeshPrimitiveCreateCylinderSub(mesh, rad, hlen, detail, 0);
}

void dynClothMeshPrimitiveCreateBalloon(DynClothMesh *mesh, F32 rad, F32 hlen, int detail)
{
	dynClothMeshPrimitiveCreateCylinderSub(mesh, rad, hlen, detail, 1);
}

void dynClothMeshPrimitiveCreateSphere(DynClothMesh *mesh, F32 rad, int detail)
{
	dynClothMeshPrimitiveCreateCylinderSub(mesh, rad, 0.0f, detail, 1);
}

void dynClothMeshPrimitiveCreatePlane(DynClothMesh *mesh, F32 rad, int detail)
{
	int i;
	Vec3 clr;
	...
	{
	int sidx = 0;
	dynClothMeshCreateStrips(mesh, 1 , CLOTHMESH_TRISTRIP);
	dynClothStripCreateIndices(&mesh->Strips[sidx], 4, -1);
...
void dynClothMeshPrimitiveCreateBox(DynClothMesh *mesh, F32 dx, F32 dy, F32 dz, int detail)
{
	int i;
	...
	{
	int sidx = 0;
	dynClothMeshCreateStrips(mesh, 6 , CLOTHMESH_TRISTRIP);
	dynClothStripCreateIndices(&mesh->Strips[sidx], 4, -1);
	mesh->Strips[sidx].IndicesCCW[0] = 0;
