	...
	for(i = 0; i < ARRAY_SIZE(chunk->columns); i++){
		...
		for(area = column->areas; area; area = area ? area->nextColumnArea : column->areas){
			...
			if(area->nextColumnArea){
				...
				//	printf("");
				//}
				area->y_max = 15000;//scene_info.maxHeight;
			}

			area->y_min_S16 = floor(area->y_min);
			area->y_max_S16 = ceil(area->y_max);

			if(	area->y_min < -15000 || //min(-2000, scene_info.minHeight) ||
				area->y_min > 15000 - 5) //scene_info.maxHeight - 5)
			{
		...
		z >= 0 && z < BEACON_GENERATE_CHUNK_SIZE)
	{
		...
		if(sideColumn->propagate.flatArea == flatArea){
			...
			if(sideArea){
				...
				switch(area->slope){
					case BGSLOPE_FLAT:{
						if( diff_S16 > BEACONCONFIGVAR(MaxYDiffS16) ||
							diff_S16 < -BEACONCONFIGVAR(MaxYDiffS16) ||
							fabs(sideArea->y_min - area->y_min) > BEACONCONFIGVAR(MaxYDiff) * scale ||
							sideArea->y_min - flatArea->y_min > BEACONCONFIGVAR(MaxFlatYDiff) ||
							flatArea->y_max - sideArea->y_min > BEACONCONFIGVAR(MaxFlatYDiff))
						{
							return 0;
						}
						break;
								...
								BeaconGenerateColumnArea* startArea)
{
	...
	if(startArea->slope != BGSLOPE_VERTICAL){
		...
		flatArea->y_min = startArea->y_min;
		flatArea->y_max = startArea->y_min;
			...
			if(!(curArea->foundSides & (1 << dir_index))){
					...
					side_z >= 0 && side_z < BEACON_GENERATE_CHUNK_SIZE)
				{
					...
					if(!flatArea || sideColumn->propagate.flatArea == flatArea){
						...
						//assert(fabs(sideColumn->propagate.curArea->y_min - curArea->y_min) <= beaconGetConfigVar(BCNCFG_GEN_MAX_Y_DIFF));
								...
								if(sideArea->slope == curArea->slope){
									switch(curArea->slope){
										case BGSLOPE_FLAT:{
											if(	diff_S16 <= BEACONCONFIGVAR(MaxYDiffS16) &&
												diff_S16 >= -BEACONCONFIGVAR(MaxYDiffS16) &&
												fabs(sideArea->y_min - curArea->y_min) <= BEACONCONFIGVAR(MaxYDiff) * scale &&
												sideArea->y_min - flatArea->y_min < BEACONCONFIGVAR(MaxFlatYDiff) &&
												flatArea->y_max - sideArea->y_min < BEACONCONFIGVAR(MaxFlatYDiff))
											{
...
static BeaconGenerateEdgeNode* sortEdgeNodesClockwise(BeaconGenerateChunk* chunk, BeaconGenerateEdgeNode* startNode){
	...
	while(cur)
	{
		...
		diff[0] = cur->columnArea->beacon->beaconPos[0] - bestNode->columnArea->beacon->beaconPos[0];
		diff[2] = cur->columnArea->beacon->beaconPos[2] - bestNode->columnArea->beacon->beaconPos[2];
...
static void sendBestCorners(BeaconGenerateEdgeNode* farNode1, BeaconGenerateEdgeNode* farNode2){
	addCombatBeacon(farNode1->columnArea->beacon->beaconPos, 1, 1, 1, 0);
	
	if(farNode1 != farNode2){
		addCombatBeacon(farNode2->columnArea->beacon->beaconPos, 1, 1, 1, 0);
	}
...
static void sendEdge(BeaconGenerateChunk* chunk, BeaconGenerateEdgeNode* startNode){
	...
	BeaconGenerateEdgeNode *saveNode, *repairNode;
	U32 color = 0xff808080 | ((rand()&127) << 16) | ((rand()&127) << 8) | (rand()&127);
	F32 y_offset = (rand() % 101) * 0.01;

	curNode = startNode->nextYawNode;
	saveNode = curNode;
	repairNode = startNode;
	startNode->nextYawNode = NULL;
	startNode = curNode;

	BEACON_CLIENT_PACKET_CREATE(BMSG_C2ST_DEBUG_MSG);
	pktSendBits(pak, 32, BDO_GEN);

	while(curNode){
		BeaconGenerateEdgeNode* next = curNode->nextEdgeNode ? curNode->nextEdgeNode : curNode != startNode ? startNode : NULL;
		...
		if(next){
			...
			SEND_LINE_XYZ(curNode->columnArea->beacon->beaconPos[0], 
	...
	repairNode->nextYawNode = saveNode;
						...
						S32 quiet)
{
	...
	while(1){
		...
		if(curNode){
			...
			curNode->nextEdgeNode->prevEdgeNode = curNode;
...
static U32 addGeneratedBeacon(int iPartitionIdx, Vec3 pos, S32 noGroundConnections, S32 color){
	...
	if(beacon_client.debug_state && distance3Squared(pos, beacon_client.debug_state->debug_pos)<SQR(4))
		printf("");

	copyVec3(pos, orig_pos);
	copyVec3(pos, pos_cast);
	pos_cast[1] += 5;

	beaconCapsuleCollide(beaconGetActiveWorldColl(iPartitionIdx), pos_cast, pos, WC_FILTER_BIT_MOVEMENT, &results);
	
	if(results.hitSomething)
	{
		if(distance3SquaredXZ(results.posWorldImpact, pos)>SQR(0.2))
		{
			// Hit some obstruction.  Let's move!
			//copyVec3(results.pos, pos);
			//pos[1] += 2;  // Get it back off the ground
			create = 0;
		}
		else if(fabs(results.posWorldImpact[1]-pos[1])>0.1)
		{
			create = 0;
		}
		else
		{
			// Some obstruction but only slightly off the ground.
			pos_cast[1] -= results.distance<0.1?0:results.distance-0.1;
			copyVec3(pos_cast, pos);
		}
	}
	...
	if(beacon_client.debug_state && beacon_client.debug_state->send_gen_lines)
	{
		BEACON_CLIENT_PACKET_CREATE(BMSG_C2ST_DEBUG_MSG);
			pktSendBits(pak, 32, BDO_GEN);
			SEND_LINE_VEC3(pos_cast, orig_pos, color);
			if(!create)
			{
				SEND_POINT(pos, 0xFFF000F0);
			}
		BEACON_CLIENT_PACKET_SEND();
	}
...
int sortEdgeNodesDist(const BeaconGenerateEdgeNode** lhs, const BeaconGenerateEdgeNode** rhs)
{
	const BeaconGenerateEdgeNode *l = *lhs, *r = *rhs;

	if(l->distSQR < r->distSQR)
		return -1;
	else if(l->distSQR > r->distSQR)
		return 1;
	
	return ((intptr_t)(l)) - ((intptr_t)(r));
}

int findEdgeDist(BeaconGenerateEdgeNode* l, BeaconGenerateEdgeNode* r, int maxDist)
{
	BeaconGenerateEdgeNode *curNode;
	int distR, distL;
...
static __forceinline F32 nodeDistSquaredXZ(BeaconGenerateEdgeNode* l, BeaconGenerateEdgeNode* r)
{
	return distance3SquaredXZ(l->columnArea->beacon->beaconPos, r->columnArea->beacon->beaconPos);
}

static __forceinline F32 nodeDir(BeaconGenerateEdgeNode* start, BeaconGenerateEdgeNode* end, Vec3 dirOut)
{
	subVec3(end->columnArea->beacon->beaconPos, start->columnArea->beacon->beaconPos, dirOut);
	return normalVec3(dirOut);
}

static int findNodeEdgeDir(BeaconGenerateEdgeNode* startNode, Vec3 dirOut, BeaconGenerateEdgeNode **targetNodeOut, int lenSQR, int next)
{
	BeaconGenerateEdgeNode *curNode = startNode;

	while(curNode!=startNode && nodeDistSquaredXZ(curNode, startNode)<lenSQR)
		curNode = next ? curNode->nextEdgeNode : curNode->prevEdgeNode;
	...
	if(targetNodeOut)
		*targetNodeOut = curNode;

	nodeDir(curNode, startNode, dirOut);

	return 1;
}
			
static void makeBeaconsAlongEdge(int iPartitionIdx, BeaconGenerateColumnArea** placedBeacons, BeaconGenerateChunk* chunk, BeaconGenerateEdgeNode* startNode){
	BeaconGenerateEdgeNode* curStartNode = startNode;
	
	while(curStartNode){
		BeaconGenerateEdgeNode* curNode;
		BeaconGenerateEdgeNode* prevNode = curStartNode;
		BeaconGenerateEdgeNode* perpNode = NULL;
		...
		if(!curStartNode->columnArea->makeBeacon){
			...
			*placedBeacons = area;
			...
			addGeneratedBeacon(iPartitionIdx, area->beacon->beaconPos, area->noGroundConnections, 0xFF00FF00);
		
			//addCombatBeacon(area->beacon->beaconPos, 1, 1);
		}

		if(perpNode && curStartNode==perpNode->nextEdgeNode)
			perpNode = NULL;
		
		for(curNode = curStartNode->nextEdgeNode; curNode; curNode = curNode->nextEdgeNode){
			Vec3 curDir, forwardDir;
			...
			if(!perpNode)
			{
				BeaconGenerateEdgeNode *newPerpNode;
				nodeDir(curStartNode, curNode, curDir);
				if(findNodeEdgeDir(curNode, forwardDir, &newPerpNode, 5, 1) &&
					fabs(dotVec3(curDir, forwardDir))<0.15)
				{
					BeaconGenerateColumnArea* area = curStartNode->columnArea;
					...
					*placedBeacons = area;
					...
					addGeneratedBeacon(iPartitionIdx, area->beacon->beaconPos, area->noGroundConnections, 0xFFFFFF00);

					perpNode = newPerpNode;
...
static void makeBeaconsInFlatArea(	int iPartitionIdx,
									...
									S32 quiet)
{
	...
	if(	flatArea->edgeList &&
		...
		flatArea->edgeList->slope != BGSLOPE_STEEP)
	{
		return;
	}
	...
	for(area = flatArea->edgeList; area; area = area->nextInFlatAreaSubList){
		...
		while(area->usedSides != 0xff){
			...
			if(tailNode && nodeCount > 1){
				...
				if(beacon_client.debug_state && beacon_client.debug_state->send_gen_edge){
				...
				startNode->prevEdgeNode = tailNode;
				
				if(!quiet){
					printf("m");
				}
				
				makeBeaconsAlongEdge(iPartitionIdx, &placedBeacons, chunk, farNode1);

				tailNode->nextEdgeNode = NULL;
				startNode->prevEdgeNode = NULL;
			}
	...
	if(!beaconGetDebugVar("nocenterfill")){
		...
		
		{
			...
			for(i = 0; i < sortCount; i++){
				...
				if(foundAreaCount){
					...
					//printf(	"\n%5d, %5d, %5d, %5d, %5d",
					//		remaining, center_x, center_z, edgeDistSQR, foundAreaCount);
					foundCount++;
				}else{
					U64 endTime;
					GET_CPU_TICKS_64(endTime);
			...
			if(!quiet){
				printf(	"\n"
						"Found:    %"FORM_LL"u ticks for %d found (%"FORM_LL"u ticks/check)\n"
						"NotFound: %"FORM_LL"u ticks for %d not found (%"FORM_LL"u ticks/check)\n",
						ticksFound,
						foundCount,
		...
		
		{
			...
			F32 cutOffMin = BEACONCONFIGVAR(FlatAreaCircleCutoffMin);
			F32 cutOffMax = BEACONCONFIGVAR(FlatAreaCircleCutoffMax);
			
			if(!quiet){
				printf("f");
			}

			if(!stashAddressFindInt(beaconGeoProximityStashProcess, flatArea->chunk->swapBlock, NULL))
			{
				cutOffMin = 256;
				cutOffMax = 512;
			}

			if(cutOffDistSQR < SQR(cutOffMin)){
				cutOffDistSQR = SQR(cutOffMin);
			}
			else if(cutOffDistSQR > SQR(cutOffMax)){
				cutOffDistSQR = SQR(cutOffMax);
			}
			...
			while(1){
				...
				if(area && maxDistSQR > cutOffDistSQR){
					...
					addGeneratedBeacon(iPartitionIdx, pos, area->noGroundConnections, 0xFF0000FF);
...
void beaconMakeBeaconsInChunk(int iPartitionIdx, BeaconGenerateChunk* chunk, S32 quiet){
	static AreaListGrid *localEdgeLists = NULL;
	BeaconGenerateFlatArea* flatArea;
	...
	localEdgeLists = calloc(1, sizeof(AreaListGrid));
	...
	for(flatArea = chunk->flatAreas; flatArea; flatArea = flatArea->nextChunkArea){
		if(!flatArea->edgeList->doNotMakeBeaconsInVolume){
			makeBeaconsInFlatArea(iPartitionIdx, chunk, flatArea, quiet);
		}else{
...
static void makeBeaconsInChunks(int iPartitionIdx){
	...
	for(swapBlock = bp_blocks.list; swapBlock; swapBlock = swapBlock->nextSwapBlock){
		...
		beaconMakeBeaconsInChunk(iPartitionIdx, swapBlock->chunk, 0);
	}
...
static void sendLinesToClient(){
		...
		for(i = 0; i < legals.count; i++){
			...
			if(srcArea && dstArea && srcArea->flatArea != dstArea->flatArea && !dstArea->flatArea->inBoundConn){
				...
				if(srcArea->flatArea->inBoundConn){
					LegalConn* l2 = srcArea->flatArea->inBoundConn;
					BeaconGenerateColumn* dstColumn2 = l2->dst.block->chunk->columns + l2->dst.columnIndex;
					BeaconGenerateColumnArea* dstArea2 = dstColumn2->areas;
					
					while(dstArea2 && dstArea2->columnAreaIndex != l2->dst.areaIndex)
						dstArea2 = dstArea2->nextColumnArea;
						
					if(dstArea2){
						Vec3 dstPos2 = {l2->dst.block->x + 0.5 + (dstColumn2 - dstColumn2->chunk->columns) % BEACON_GENERATE_CHUNK_SIZE,
										dstArea2->y_min + 0.1,
										l2->dst.block->z + 0.5 + (dstColumn2 - dstColumn2->chunk->columns) / BEACON_GENERATE_CHUNK_SIZE};
										
						SEND_LINE(	srcPos[0], srcPos[1], srcPos[2],
									dstPos2[0], dstPos2[1], dstPos2[2],
									0x80ff8000);
					}
				}
			}
	...
	if(beaconGetDebugVar("sendbeacons")){
		for(i = 0; i < combatBeaconArray.size; i++){
			...
			SEND_BEACON(vecX(b->pos), vecY(b->pos) - 2, vecZ(b->pos), 0xffff8000);
		}
	}
						...
						for(j = 0; j < 8; j++){
							if(columnArea->sides[j].area && columnArea->sides[j].area->isEdge){
								...
								if(good[0] + good[1] >= 1){
									...
									if((intptr_t)columnArea < (intptr_t)columnArea->sides[j].area){
...
static void addNewArea(S32 x, S32 z, F32 y_min, F32 y_max){
	...
	#if BEACONGEN_CHECK_VERTS
	{
		...
		column->tris.tris[i].smd = currentTriVerts.smd;
	...
	for(area = column->areas; area; area = area->nextColumnArea){
		...
		if(!used && y_max + 6 <= area->y_min){
			...
			if(y_max > area->y_max){
				if((U32)curRender.slope < area->slope || y_max - area->y_max > BEACONCONFIGVAR(BetterSlopeYDiff)){
			...
			else if((U32)curRender.slope < area->slope && area->y_max - y_max < BEACONCONFIGVAR(BetterSlopeYDiff)){
				area->slope = curRender.slope;
			}
			...
			if(area->y_max > prevArea->y_max){
				if(area->slope < prevArea->slope || area->y_max - prevArea->y_max > BEACONCONFIGVAR(BetterSlopeYDiff)){
			...
			else if(area->slope < prevArea->slope && prevArea->y_max - area->y_max < BEACONCONFIGVAR(BetterSlopeYDiff)){
				prevArea->slope = area->slope;
			}
...
static S32 renderTriangleToChunks(Vec3 verts[3], Vec3 normal){
	...
	assert(lengthVec3(normal) <= 1.001);
	...
	qsort(verts, 3, sizeof(verts[0]), cmpVec3ZX);
...
// 	if(verts[0][2] >= 40 && verts[2][2] <= 55 && min_x >= 36 && max_x <= 63)
// 		printf("");

	dz = vecZ(verts[2]) - vecZ(verts[0]);
	
	for(i=0; i<BGSLOPE_COUNT; i++)
	{
		F32 limit = BEACONSLOPELIMIT(i);
		if(normal_y > limit)
		{
			curRender.slope = i;
			break;
...
static void extractTrianglesFromModel(	WorldCollStoredModelData* smd,
										WorldCollModelInstanceData *inst)
{
	S32						pointCount = 0;
	S32						i;
	Vec3					min, max;

	curRender.noGroundConnections = inst->noGroundConnections;
	
	//printf("model: %s (%d tris) at (%.1f, %.1f, %.1f)\n", smd->name, smd->tri_count, vecParamsXYZ(mat[3]));
	
// 	if(!stricmp(smd->name, "XMElevatorRoom_HTech_floor_01"))
// 		gPrintf = true;

	beaconCalcSMDMatMinMaxSlow(smd, inst->world_mat, min, max);

#if BEACONGEN_CHECK_VERTS
	currentTriVerts.smd = smd;
#endif
	...
	if(smd->tri_count)
	{
		for(i = 0; i < smd->tri_count; i++){
			...
			mulVecMat3(smd->norms[i], inst->world_mat, normal_world);

			for(j = 0; j < 3; j++){
				mulVecMat4(smd->verts[tri[j]], inst->world_mat, verts_world[j]);
			}

			normalVec3(normal_world);
			pointCount += renderTriangleToChunks(verts_world, normal_world);
		}
	}
	else if(smd->map_size)
	{
		...
		x_step = smd->grid_size / ((F32)smd->map_size-1);
		z_step = smd->grid_size / ((F32)smd->map_size-1);
		for(i=0; i < smd->map_size-1; i++)
		{
			z = smd->min[2];
			for(j=0; j < smd->map_size-1; j++)
			{
				Vec3 d1, d2;

				// Set up this vertex
				h1 = smd->heights[j*smd->map_size+i];
				h2 = smd->heights[j*smd->map_size+(i+1)];
				h3 = smd->heights[(j+1)*smd->map_size+i];
				h4 = smd->heights[(j+1)*smd->map_size+(i+1)];

				x = smd->min[0]+i*x_step;
				z = smd->min[2]+j*z_step;

				setVec3(t1, x, h1, z);
				setVec3(t2, x, h2, z+z_step);
				setVec3(t3, x+z_step, h3, z);
				setVec3(t4, x+z_step, h4, z+z_step);

				mulVecMat4(t1, inst->world_mat, p1);
				mulVecMat4(t2, inst->world_mat, p2);
				mulVecMat4(t3, inst->world_mat, p3);
				mulVecMat4(t4, inst->world_mat, p4);
				/*
				assert(pointBoxCollision(p1, min, max));
				assert(pointBoxCollision(p2, min, max));
				assert(pointBoxCollision(p3, min, max));
				assert(pointBoxCollision(p4, min, max));
				*/

				copyVec3(p1, verts_world[0]);
				copyVec3(p2, verts_world[1]);
				copyVec3(p3, verts_world[2]);

				subVec3(p2, p1, d1);
				subVec3(p3, p1, d2);
				crossVec3(d1, d2, normal_world);
				normalVec3(normal_world);
				pointCount += renderTriangleToChunks(verts_world, normal_world);

				copyVec3(p2, verts_world[0]);
				copyVec3(p4, verts_world[1]);
				copyVec3(p3, verts_world[2]);
				subVec3(p4, p2, d1);
				subVec3(p3, p2, d2);
				crossVec3(d1, d2, normal_world);
...
static void beaconExtractSurfacesWorldCollHelper(	void* userPointer,
													const WorldCollObjectTraverseParams* params)
{
	...
	{
		assert(inst);
		extractTrianglesFromModel(smd, inst);
	}

	SAFE_FREE(inst);
}

void beaconExtractSurfaces(WorldColl *wc, S32 grid_x, S32 grid_z, S32 dx, S32 dz, S32 justEdges){
	S32 remaining = dx * dz;
	...
	{
		Vec3 min_xyz = {min_tri_x, 0, min_tri_z};
		Vec3 max_xyz = {max_tri_x, 0, max_tri_z};
		
		wcTraverseObjects(	wc,
							beaconExtractSurfacesWorldCollHelper,
							NULL,
							min_xyz,
							max_xyz,
							/*unique=*/1,
...
void beaconGenerateInitBlocks(S32 quiet)
{	
	S32 x, z;
	...
	bp_blocks.grid_min_xyz[0] = floor(beacon_process.world_min_xyz[0] / BEACON_GENERATE_CHUNK_SIZE);
	bp_blocks.grid_min_xyz[1] = floor(beacon_process.world_min_xyz[1] / BEACON_GENERATE_CHUNK_SIZE);
	bp_blocks.grid_min_xyz[2] = floor(beacon_process.world_min_xyz[2] / BEACON_GENERATE_CHUNK_SIZE);
	
	bp_blocks.grid_max_xyz[0] = floor(beacon_process.world_max_xyz[0] / BEACON_GENERATE_CHUNK_SIZE);
	bp_blocks.grid_max_xyz[1] = floor(beacon_process.world_max_xyz[1] / BEACON_GENERATE_CHUNK_SIZE);
	bp_blocks.grid_max_xyz[2] = floor(beacon_process.world_max_xyz[2] / BEACON_GENERATE_CHUNK_SIZE);

	if(!quiet)
		printf("Min: %.3f %.3f %.3f | Max: %.3f %.3f %.3f\n", vecParamsXYZ(beacon_process.world_min_xyz), vecParamsXYZ(beacon_process.world_max_xyz));
	...
	{
		// Bad or empty map
		bp_blocks.grid_min_xyz[0] = 0;
		bp_blocks.grid_min_xyz[1] = 0;
		bp_blocks.grid_min_xyz[2] = 0;

		bp_blocks.grid_max_xyz[0] = -1;
		bp_blocks.grid_max_xyz[1] = -1;
		bp_blocks.grid_max_xyz[2] = -1;
		return;
	}

	for(x = bp_blocks.grid_min_xyz[0]; x <= bp_blocks.grid_max_xyz[0]; x++){
		for(z = bp_blocks.grid_min_xyz[2]; z <= bp_blocks.grid_max_xyz[2]; z++){
