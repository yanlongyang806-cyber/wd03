								 ...
								 const char *channel_name, ChatUser *user, ChatUser *target, const char *resultString)
{
	...
	objLog(LOG_CHATSERVER, GLOBALTYPE_CHATUSER, userID, 0, handle, NULL, user ? user->accountName : NULL, commandString, NULL, 
		"[Channel:%s][Tar:%d-%s] %s", channel_name, targetID, targetHandle, resultString);
	if (channel)
	{
		objLog(LOG_CHATSERVER, GLOBALTYPE_CHATCHANNEL, channel->uKey, channel->uKey, channel->name, NULL, channel->name, commandString, NULL, 
			"[User:%d-%s][Tar:%d-%s] %s", userID, handle, targetID, targetHandle, resultString);
	}
	
	if (target && converseCommandString)
	{
		objLog(LOG_CHATSERVER, GLOBALTYPE_CHATUSER, targetID, 0, handle, NULL, target->accountName, converseCommandString, NULL, 
