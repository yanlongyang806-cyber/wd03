...
void csrSilenceUser(ChatUser *user, U32 duration)
{
	userSilence(user, duration);
}

AUTO_COMMAND ACMD_ACCESSLEVEL(4) ACMD_CATEGORY(ChatAdmin) ACMD_NAME(csrSilence);
void csrSilenceInternal(ContainerID targetID, U32 duration)
{
	ChatUser *target = userFindByContainerId(targetID);
	if (target)
	{
		csrSilenceUser(target, duration);
	}
}
...
void csrUnsilenceUser(ChatUser *user)
{
	userUnsilence(user, true);
}

AUTO_COMMAND ACMD_ACCESSLEVEL(4) ACMD_CATEGORY(ChatAdmin) ACMD_NAME(csrUnsilence);
void csrUnsilenceInternal(ContainerID targetID)
{
	ChatUser *target = userFindByContainerId(targetID);
	if (target)
	{
		csrUnsilenceUser(target);
	}
}
...
int csrUnsilenceByDisplayName(CmdContext *context, ContainerID userID, const ACMD_SENTENCE displayName)
{
	...
	if (csrUserIsGM(user))
	{
		...
		target = userFindByHandle(displayName);
		csrUnsilenceInternal(target->id);
	}
	return CHATRETURN_FWD_NONE;
}
...
{
	channelLeave(user, channel->name, true);
}
...
{
	if (user && channel)
		channelJoin(user, channel->name, 0);
}
...
void csrSetUserAccessLevel(ChatChannel *channel, ChatUser *user, char *options)
{
	...
	/*if (watching)
	{
	parseAccessString((ChannelAccess*) &watching->access, options);
	}*/
	// TODO does nothing?
}
...
void csrSetChannelAccessLevel(ChatChannel *channel, char *options)
{
	parseAccessString((ChannelAccess*) channel->access, options);
}
AUTO_COMMAND ACMD_CATEGORY(ChatAdmin) ACMD_NAME(csrChannelAccess);
void csrSetChannelAccessInternal(char *channel_name, ACMD_SENTENCE options)
{
	ChatChannel *channel = channelFindByName(channel_name);
	if (channel)
	{
		csrSetChannelAccessLevel(channel, options);
	}
}
AUTO_COMMAND_REMOTE ACMD_ACCESSLEVEL(4) ACMD_CATEGORY(ChatAdmin) ACMD_NAME(csrChannelAccess);
void csrSetChannelAccessCommand(ContainerID userID, char *channel_name, ACMD_SENTENCE options)
{
	ChatUser *user = userFindByContainerId(userID);
	if (csrUserIsGM(user))
	{
		csrSetChannelAccessInternal(channel_name, options);
	}
}
