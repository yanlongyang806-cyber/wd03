...
// XMPP domain classification of a channel
AUTO_ENUM;
typedef enum XMPP_RoomDomain {
	XMPP_RoomDomain_Unknown = -1,
	XMPP_RoomDomain_Channel,
	XMPP_RoomDomain_Guild,
	XMPP_RoomDomain_Officer,
	XMPP_RoomDomain_Max
} XMPP_RoomDomain;

// Outcome of channel join request
AUTO_ENUM;
typedef enum XMPP_ChannelJoinStatus {
	XMPP_ChannelJoinStatus_Success = 0,			// Joined channel successfully
	XMPP_ChannelJoinStatus_UnknownFailure,		// Unable to join channel for unknown reason
	XMPP_ChannelJoinStatus_MaxChannels,			// The user is at the maximum channels per user.
	XMPP_ChannelJoinStatus_Full,				// The channel is full.
	XMPP_ChannelJoinStatus_InvalidName,			// The channel has an invalid name.
	XMPP_ChannelJoinStatus_PermissionDenied,	// User is not allowed to join the channel because of permissions (eg. not in the guild, or private channel)
} XMPP_ChannelJoinStatus;
