...
{ if (!n)
  ...
  if(soap->mode & SOAP_IO_CRYPTIC_STRING)
  {
	  estrConcat(soap->estring, s, (unsigned int)n);
	  return SOAP_OK;
  }
...
zlib_again:
  ...
  if (soap->mode & SOAP_IO_CRYPTIC_STRING)
  {
	  ...
	  else if(len < SOAP_BUFLEN)
	  {
		  // remaining data fits in buffer, fill it up
		  memcpy(soap->buf, soap->input, len);
		  soap->input += len;
		  soap->bufidx = 0;
		  soap->buflen = len;
		  return SOAP_OK;
	  ...
	  {
		  // too much data, fill up the buffer 
		  memcpy(soap->buf, soap->input, SOAP_BUFLEN);
		  soap->input += SOAP_BUFLEN;
		  soap->bufidx = 0;
...
{ ok = ssl_verify_callback(ok, store);
  ...
  {
#ifdef SOAP_DEBUG
    printf("ignoring certificate expiration\n");
#endif
...
{ register int i;
  ...
  { for (mp = soap->mht[i]; mp; mp = mq)
    { mq = mp->next;
      if (mp->live)
        printf("%s(%d): malloc() = %p not freed (memory leak or forgot to call soap_end()?)\n", mp->file, mp->line, mp->ptr);
      free(mp);
    }
    soap->mht[i] = NULL;
  }
...
{ register size_t h = soap_hash_ptr(p);
  ...
  { if (mp->live)
    { free(p);
      ...
      printf("%s(%d): free(%p) double free of pointer malloced at %s(%d)\n", file, line, p, mp->file, mp->line);
  }
  else
    printf("%s(%d): free(%p) pointer not malloced\n", file, line, p);
}
#endif
...
{ if (soap_check_state(soap))
  ...
  { register char **q;
    ...
    { 
      ...
      {
#ifdef SOAP_MEM_DEBUG
        printf("Data corruption in dynamic allocation (see logs)\n");
#endif
...
#ifdef SOAP_MEM_DEBUG
          printf("new(object type = %d) = %p not freed: deletion callback failed\n", q->type, q->ptr);
#endif
        }
        SOAP_FREE(soap, q);
        return;
